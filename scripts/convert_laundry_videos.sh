...
#
# Usage:
#   bash scripts/convert_laundry_videos.sh <input_file> <output_file> [speed] [target_mb]
#
# Arguments:
...
#
# Example:
#   bash scripts/convert_laundry_videos.sh input.MOV output.mp4 2 8
...
if echo "$COLOR_TRC" | grep -qE "smpte2084|arib-std-b67|bt2020"; then
  ...
  VF="setpts=PTS/${SPEED},zscale=t=linear:npl=1000,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=reinhard:peak=1000:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p,eq=gamma=1.15"
else
